...
JSON Server — utilizado como base da API REST (db.json), responsável por armazenar e servir as avaliações dos usuários. Escolhido por permitir prototipar rapidamente o backend de avaliações sem a necessidade de configurar um servidor real, mantendo o projeto leve.

Web APIs nativas do navegador (navigator.userAgent, navigator.language, navigator.connection, window.matchMedia, screen) — utilizadas para construir o banner de detecção de ambiente, dispensando bibliotecas externas e mantendo a funcionalidade leve e nativa.
